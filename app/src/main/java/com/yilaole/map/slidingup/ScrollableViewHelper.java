package com.yilaole.map.slidingup;
...
/**
 * 上拉的自定义监听需要在这里配置，布局中需要添加属性：xxx:umanoScrollableView="@+id/xxx"
 */
...
public class ScrollableViewHelper {
    ...
    public int getScrollableViewScrollPosition(View scrollableView, boolean isSlidingUp) {
        ...
        } else if (scrollableView instanceof DropDownMenuMap) {//颐福来app中自定义的View，使用者可以删除该无用代码
            if (isSlidingUp) {
                MLog.d("ScrollableViewHelper---------------------true");
                return scrollableView.getScrollY();
            } else {
                MLog.d("ScrollableViewHelper---------------------false");
                DropDownMenuMap dropDownMenuMap = ((DropDownMenuMap) scrollableView);
                View child = dropDownMenuMap.getChildAt(0);
                return (child.getBottom() - (dropDownMenuMap.getHeight() + dropDownMenuMap.getScrollY()));
            }
        } else {
            return 0;
        }
